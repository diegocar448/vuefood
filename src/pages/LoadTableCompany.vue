...
export default {
    ...
    this.getCompanyByToken(this.token_company)
      .then((response) => {
        ...
        this.getTableFromCompany(params)
          .then((response) =>
            this.$vToastify.success("Loja carregada com sucesso", "Sucesso")
          )
          .catch((response) => {
            this.$vToastify.error("Falha ao Carregar Empresa", "Error");
          })
          .finally((response) =>
            this.$router.push({
              name: "products",
              params: { companyFlag: this.flag },
            })
          );
      })
      .catch((response) => {
  ...
  methods: {
    ...mapActions(["getCompanyByToken", "getTableFromCompany"]),
  },
};
</script>